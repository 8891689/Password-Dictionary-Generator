...
void generateDictionary(long long numPasswords, int minLength, int maxLength, int threads, int random, FILE *file, const char *charset, int charsetLength, long long possiblePasswords, int infinite) {
    ...
    ThreadData *threadData = malloc(sizeof(ThreadData) * threads);
    if(!threadData) {
        perror("内存分配失败");
        free(threadIds);
        return;
    ...
    if(!random && !infinite) {
        ...
        cumulativeLengths = malloc(sizeof(long long) * (numLengths +1)); // +1 for starting 0
    ...
    for(int i = 0; i < threads; ++i) {
        ...
        if(pthread_create(&threadIds[i], NULL, generatePasswords, &threadData[i]) !=0) {
            ...
            free(threadIds);
            free(threadData);
            if(cumulativeLengths) free(cumulativeLengths);
            return;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if(!random) {
        // 预计算累积密码数
        cumulativeLengths = malloc(sizeof(long long) * (maxLength - minLength +1 +1)); // +1 for starting 0
        if(!cumulativeLengths) {
            perror("内存分配失败");
